    ...
    { // Структура источника конструирования, то есть структура, содержащая все данные, необходимые для
      // создания полнофункционального объекта RadioComponentInsertion.
        ...
        std::vector<GraphObj*> pin_labels;  // Набор текстовых графических элементов (типа ObjText), соответствующих подписям выводов
                                            // копии вставленного радиокомпонента. Непосредственный текст каждого примитива совпадает с
                                            // "алфавитно-цифровым номером" соответствующей ножки.
        // Данные о подсоединении выводов компонента (первый член каждой пары - имя вывода) к токопроводящим цепям схемы или платы
        // (второй член каждой пары - имя цепи).
        std::vector<std::pair<std::string, std::string>> connect_info;
        // Данные о переназначении типов ножек (контактов) для данного экземпляра компонента (IPT-данные). При отсутствии такового
        // переназначения этот массив пуст. Иначе первый член каждой пары - имя ножки (вывода), второй - вновь назначенный ей тип.
