...
enum class PCADLoadError
{
    ...
    LOAD_FILE_CREATE_ERROR,
    LOAD_FILE_OPEN_ERROR,
    LOAD_FILE_READ_ERROR,
    LOAD_FILE_WRITE_ERROR,
    ...
    LOAD_FILE_INCORRECT_PARAMS_QUANTITY,
...
struct OptionsData
{
    ...
    wxFontEncoding pdif_encoding = wxFontEncoding::wxFONTENCODING_ISO8859_1; // Используемая кодировка PDIF-данных.
};

// Список определяющих пар (первый член пары - тип кодировки - элемент перечисления wxFontEncoding, второй - текстовое
// имя этой кодировки) допустимых кодировок для PDIF-документов.
extern const std::vector<std::pair<wxFontEncoding, wxString>> pdif_supported_encodings;

std::string UpcaseString(const std::string& arg_string);
std::string TrimString(const std::string& arg_string);
std::vector<std::pair<wxFontEncoding, wxString>>::const_iterator FindEncodingPair(wxFontEncoding scan_pdif_encoding);
